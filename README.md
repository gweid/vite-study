...
Vite 在读取完配置文件（即`vite.config.ts`）之后，会拿到用户导出的配置对象，然后执行 config 钩子（**也就是配置解析前调用 config钩子**）。在这个钩子里面，可以对配置文件导出的对象进行自定义的操作
...
Vite 在**配置解析完之后会调用`configResolved`钩子**，这个钩子一般用来记录最终的配置信息，而不建议再修改配置，用法如下图所示:
...
这个钩子仅在**开发阶段**会被调用，用于扩展 Vite 的 Dev Server，可以在开发服务器启动前插入自定义中间件或逻辑，如下代码所示:
...
html 入口定制，可以用来动态修改 `index.html` 内容。这个钩子用来灵活控制 HTML 的内容，可以拿到原始的 html 内容后进行任意的转换：
...
这个钩子会在 Vite 服务端处理热更新时被调用，可以在这个钩子中拿到热更新相关的上下文信息，自定义 HMR 更新逻辑，精准控制模块热替换行为
